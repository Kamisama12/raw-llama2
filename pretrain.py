'''
预训练的脚本
github上这个项目的源代码太乱了，经过了两次甚至更多的搬运，有一些没有用上的设置也保留了，对我这种没有对pytorch方法烂熟于心的人来
说太费劲了看起来，这里做了代码简化仅仅保留最基础运行设置，一些更深层次的训练优化设置暂时去掉。
'''
import torch
import dataset
from torch.utils.data import DataLoader,random_split
from dataset import Xchatdataset
import os
import logging
import math
import time
...
def train_epoch(epoch,
    ...
    for step, (X, Y) in enumerate(loader):
        ...
        if step % log_interval == 0:
            ...
            logger.info(
                    'Rank:{}:Epoch:[{}/{}]({}/{}) loss:{:.3f} lr:{:.7f} epoch_Time:{}min:'.format(
                        rank,
                        epoch+1,
                        max_epoch, 
                        step, 
...
def init_model(mode:str,out_dir:str=None):
    ...
    if mode == "scratch":
        ...
        #     model_args.append(checkpoint_model_args[k])
        gptconf =ModelArgs(**model_args)
        model = Transformer(gptconf)
...
if __name__=='__main__':
    ...
    try:
        for epoch in range(max_epoch):
            train_epoch(epoch,loader=train_loader,model=ddp_model,optimizer=optimizer,iter_per_epoch=len(train_loader))
        ...
        if rank==0:
            ...
            # torch.save(ddp_model.module.state_dict(),'{}_epoch_{}.bin'.format(save_dir,epoch))
            torch.save(checkpoint_dict,'{}_epoch_{}_interupt.bin'.format(save_dir,epoch+1))
